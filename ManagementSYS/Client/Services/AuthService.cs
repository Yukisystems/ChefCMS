...
namespace ManagementSYS.Client.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<RegisterResult> Register(RegisterModel registerModel)
        {
            ...
            var rgisterResult = JsonSerializer.Deserialize<RegisterResult>(await response.Content.ReadAsStringAsync(), new JsonSerializerOptions { PropertyNameCaseInsensitive = true });


            //var result = await _httpClient.PostAsJsonAsync<RegisterResult>("api/accounts", registerModel);

            return rgisterResult;
        }
